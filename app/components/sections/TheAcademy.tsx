...
function ArchDecoration() {
  return (
    <svg aria-hidden="true" style={{ position: "absolute", top: 0, right: 0, width: 280, height: 280, opacity: 0.04, pointerEvents: "none" }} viewBox="0 0 280 280">
      {[0, 1, 2, 3].map((i) => (
        <circle key={i} cx="280" cy="0" r={80 + i * 40} fill="none" stroke="var(--lavender)" strokeWidth="1" />
      ))}
    </svg>
  );
}
          ...
          {CERTIFICATIONS.map((cert, i) => {
            ...
            return (
              ...
              >
                ...
                <h3 style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: "0.95rem", color: "var(--text-primary)", letterSpacing: "-0.01em", marginBottom: "0.4rem", lineHeight: 1.3 }}>
                  {cert.title}
                </h3>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "1rem" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "0.35rem" }}>
                    ...
                    <span style={{ fontFamily: "var(--font-mono)", fontSize: "0.65rem", color: "var(--text-muted)", letterSpacing: "0.05em" }}>{cert.date}</span>
                  </div>
                  <a href={cert.credentialUrl} target="_blank" rel="noopener noreferrer"
                    aria-label={`Verify ${cert.title} credential`}
                    style={{ display: "flex", alignItems: "center", gap: "0.3rem", fontFamily: "var(--font-mono)", fontSize: "0.62rem", color: accentColor, textDecoration: "none", letterSpacing: "0.06em", transition: "opacity 0.2s" }}
